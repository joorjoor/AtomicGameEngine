...
namespace('build', function() {
    ...
    task('android_native', {
        ...
        if (os.platform() == "win32") {
            cmds.push(atomicRoot + "Build/Scripts/Windows/CompileAndroid.bat " + config["config"] + " armeabi-v7a");
        }
        else {
	...
	task('android_native64', {
        ...
        if (os.platform() == "win32") {
            cmds.push(atomicRoot + "Build/Scripts/Windows/CompileAndroid.bat " + config["config"] + " arm64-v8a");
        }
        else {
            cmds.push("cmake -G \"Unix Makefiles\" -DCMAKE_TOOLCHAIN_FILE=../../../Build/CMake/Toolchains/android.toolchain.cmake -DCMAKE_BUILD_TYPE="  + config["config"] + " -DANDROID_ABI=arm64-v8a ../../../");
            cmds.push("make -j4");
        }
